...
def normalization(x_train, mean = None, std = None):
    '''
        Normalization function taken from Assigment 2 of ISMI course 2022/2023
    ''' 
    ...
    if mean == None and std == None:
        ...
        for feature in range(0,len(x_train[0])):
            ...
            std.append(np.std(x_train[:,feature], dtype=x_train[:,feature].dtypes)) # array of std value per feature

        for x in range(len(x_train)):
            for i, (mu, sigma) in enumerate(zip(mean, std)):
                x_train_normalized[x, i] = ((x_train[x,i]-mu)/sigma)
        return pandas.DataFrame(x_train_normalized), mean, std
    else:
        for x in range(len(x_train)):
            for i, (mu, sigma) in enumerate(zip(mean, std)):
                x_train_normalized[x, i] = ((x_train[x,i]-mu)/sigma)
        return pandas.Dataframe(x_train_normalized)
...
def get_data_loader(
    ...
    if train: 
        ...
        # dataset_translation.dataset = normalization(dataset_translation.dataset, train_mean, train_std)
        ...
        # dataset_rotation.dataset = normalization(dataset_rotation.dataset, train_mean, train_std)
        ...
        # dataset_trans_rot.dataset = normalization(dataset_trans_rot.dataset, train_mean, train_std)
    ...
    if train == False:
        ...
        # dataset = normalization(dataset.dataset, mean, std)
...
def enhance_contrast(patch):
    # https://staff.fnwi.uva.nl/r.vandenboomgaard/IPCV20172018/LectureNotes/IP/PointOperators/HistogramEqualization.html
    his, be = np.histogram(patch, range=(0,1), bins=100)
    his = his.astype(float)/sum(his)
    image_equalized = np.interp(patch, be, np.hstack((np.zeros((1)), np.cumsum(his))))
    return image_equalized.reshape(patch.shape)
    # number_bins = 256 #https://web.archive.org/web/20151219221513/http://www.janeriksolem.net/2009/06/histogram-equalization-with-python-and.html
    # image_histogram, bins = np.histogram(patch.flatten(), number_bins, density=True)
    # cdf = image_histogram.cumsum() # cumulative distribution function
    # cdf = (number_bins-1) * cdf / cdf[-1] # normalize

    # # use linear interpolation of cdf to find new pixel values
    # image_equalized = np.interp(patch.flatten(), bins[:-1], cdf).astype(float)
