...
class NoduleDataset(data.Dataset):
    ...
    def _extract_patch(
        ...
        _, y, x = np.array(self.patch_size) // 2
        _, py, px = self.patch_size
        half_y = int(0.5*py)
        half_x = int(0.5*px)
        patch = patch[int(y-half_y):int(y+(py-half_y)),int(x-half_x):int(x+(px-half_x))] / np.mean(patch[int(y-half_y):int(y+(py-half_y)),int(x-half_x):int(x+(px-half_x))])
        mask = mask[int(y-half_y):int(y+(py-half_y)),int(x-half_x):int(x+(px-half_x))] / 255
...
def enhance_contrast(patch):
    ...
    image_histogram, bins = np.histogram(patch.flatten(), 100, density=True)
    cdf = image_histogram.cumsum() # cumulative distribution function
    cdf = (100-1) * cdf / cdf[-1] # normalize
